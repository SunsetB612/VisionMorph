...
def create_generation_task(request: GenerationRequest, db: Session = Depends(get_db)):
    """创建生成任务"""
    try:
...
def get_generated_images_list(original_image_id: int, db: Session = Depends(get_db)):
    """获取生成图片列表"""
